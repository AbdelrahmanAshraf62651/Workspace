    ...
    <div className="container pt-5">
      ...
      <div className="row g-4">
        {drinks.map((drink, index) => (
          <CafeCard title={drink.name} description={drink.description} price={drink.price.toString()} image={drink.image} key={index} inStock={drink.in_stock} />
        ))}
      </div>
      <h2 className="fw-semibold pb-2 sub-title mt-5">Food</h2>
      <div className="row g-4">
        {food.map((foodItem, index) => (
          <CafeCard title={foodItem.name} description={foodItem.description} price={foodItem.price.toString()} image={foodItem.image} key={index} inStock={foodItem.in_stock} />
        ))}
      </div>
    </div>
  );
